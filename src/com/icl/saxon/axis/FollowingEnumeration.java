...
class FollowingEnumeration extends AxisEnumeration {
    
    private NodeInfo root;
    
    public FollowingEnumeration(NodeInfo node, int nodeType, NameTest nodeName)
    throws SAXException {
        super(node, nodeType, nodeName);
        root = node.getDocumentRoot();
        // skip the descendant nodes if any
        do {
            next = (NodeInfo)node.getNextSibling();
            if (next==null) node = (NodeInfo)node.getParentNode();
